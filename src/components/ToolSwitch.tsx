...
export default function ToolSwitch({
  ...
  return (
    <button
      ...
      disabled={isDisabled}
      className={`
        p-2 rounded-lg transition-all duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:cursor-not-allowed hover:bg-gray-100 dark:hover:bg-gray-800
        ${
          enabled && modelSupportsTools
            ? "text-green-500 focus:ring-green-500"
            : "text-gray-600 dark:text-gray-400 focus:ring-gray-500"
        }
        ${isDisabled && (!enabled || !modelSupportsTools) ? "opacity-50" : ""}
      `}
      title={
        !modelSupportsTools
    ...
    >
      <Wrench
        className={`
          ...
          ${enabled && modelSupportsTools ? "drop-shadow-sm" : ""}
        `}
      />
    </button>
  );
